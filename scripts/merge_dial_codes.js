...
const merged = countries.map(c => {
    const dial = dialCodes.find(d => d.code === c.code);
    return {
        ...c,
        dialCode: dial ? dial.dial_code.replace(/\s/g, '') : ""
    };
});
...
export const countries: Country[] = ${JSON.stringify(merged, null, 4)};
`;

fs.writeFileSync('/home/austine/Desktop/Locations/get-locations/data/countries.ts', content);
console.log('Successfully merged dial codes.');
